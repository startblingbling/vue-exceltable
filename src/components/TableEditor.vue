    ...
    >
      ...
      <div style="flex:1;" v-show="editor.editing">
        ...
        <el-select
          ...
          clearable
          popper-class="excelSelect"
          :placeholder="editor.selectProps.placeholder"
        >
          <el-option
        ...
        <el-cascader
          ...
          :placeholder="editor.cascaderProps.placeholder"
          clearable
          collapse-tags
          v-model="editContent"
...
<script>
import { DatePicker, Select, Option, Cascader } from 'element-ui'
...
export default {
  ...
  computed: {
    ...
    cellEditorStyle () {
      ...
      return {
        top: `${this.editor.editorYIndex * this.rowHeight -
          this.store.states.tableBodyTop +
          this.theaderHeight}px`,
        left: `${left}px`,
    ...
    autofillHandlerStyle () {
      ...
      return {
        top: `${(this.autofill.autofillYIndex + 1) * this.rowHeight -
          4 -
          this.store.states.tableBodyTop +
          this.theaderHeight}px`,
        left: `${left}px`,
    ...
    selectedStyle () {
      return {
        top: `${this.selector.selectedYArr[0] * this.rowHeight -
          this.store.states.tableBodyTop +
          this.theaderHeight}px`,
        left: `${this.columnsWidth
          .filter((item, index) => index < this.selector.selectedXArr[0])
          .reduce((sum, item) => sum + item, 0) -
          this.store.states.tableBodyLeft}px`,
    ...
    autofillStyle () {
      return {
        top: `${this.autofill.autofillYArr[0] * this.rowHeight -
          this.store.states.tableBodyTop +
          this.theaderHeight}px`,
        left: `${this.columnsWidth
          .filter((item, index) => index < this.selector.selectedXArr[0])
    ...
    'editor.editing': {
      handler (val) {
        ...
        if (!val) {
          ...
          if (this.columns[this.editor.editorXIndex].type === 'cascader') {
            this.store.getEditorContent(
              this.$refs.cascader.getCheckedNodes(),
              'cascader',
              this.editContent
            )
          } else if (this.columns[this.editor.editorXIndex].type === 'select') {
            this.store.getEditorContent(
              this.$refs.select.selected,
              'select',
              this.editContent
            )
          } else if (this.columns[this.editor.editorXIndex].type === 'date') {
            this.store.getEditorContent(this.editContent, 'date')
          } else if (this.columns[this.editor.editorXIndex].type === 'month') {
            this.store.getEditorContent(this.editContent, 'month')
          } else {
