<template>
  ...
  >
    ...
    <scroll barType="y" :store="store"> </scroll>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleMousewheel () {
      ...
      this.$nextTick(() => {
        this.$refs.fixedTbody.$el.addEventListener(
          'mousewheel',
          mainWrapperWheel
        )
        this.$refs.fixedTbody.$el.addEventListener(
      ...
      for (
        ...
        for (
          ...
          if (
            ...
            content += `${states.showData[i + states.selector.selectedYArr[0]][
            ...
            content += `\t${states.showData[
              i + states.selector.selectedYArr[0]
            ][states.columns[j + states.selector.selectedXArr[0]].key] || ''}\n`
          } else {
            content += `\t${states.showData[
              i + states.selector.selectedYArr[0]
    ...
    setEditing (key) {
      ...
      states.editor.editType = states.columns[states.editor.editorXIndex].type
        ...
        : 'text'
      states.editor.curEditorWidth = this.columnsWidth[
      ...
      if (states.editor.editType === 'cascader') {
        states.editor.cascaderProps =
          states.columns[states.editor.editorXIndex].cascaderProps
        states.editor.options =
          states.columns[states.editor.editorXIndex].options
      }
      this.$nextTick(() => {
    ...
    adjustPosition () {
      ...
      if (this.maxHeight) {
        ...
        const curBottomShould =
          this.scrollTopArr[states.editor.editorYIndex + 1] - this.maxHeight + 1
