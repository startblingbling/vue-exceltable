...
class TableStore {
  ...
  calcDomData() {
    ...
    states.visibleRowEndIndex = states.visibleRowStartIndex
  ...
  // editor
  //select 和 cascader需要在它后边添加一列隐藏的单元格，用来存放value值，在本单元格存放label
  getEditorContent(text, type, val) {
    const { states } = this;
    let editContent = "";
    var checkedList = [];
    switch (type) {
  ...
  sort(type) {
    ...
    if (type === 'ascending') {
      states.showData.sort((x, y) => (x[states.columnsStatusList[states.dropdown.index].key]
  ...
  handleChangeData() {
    ...
    states.changeData = data.filter(
      (item, index) => JSON.stringify(item) !== JSON.stringify(initialData[index]),
    );
  }
